...
import getSongPath from '../../../app/backend/ipc/getSongPath';
...
describe('ipcPlaylist', () => {
  ...
  it('ipcPlaylistIntegration', async () => {
    ...
    const songStore = SongStore.getInstance();
    songStore.setDirectory(tempDir);
    ...
    // getSongPath IPC
    const songPath = await getSongPath(song1.ytID);
    expect(songPath).toEqual(path.join(tempDir, song1.fileName));

    let err0;
    try {
      await getSongPath('dummySongId');
    } catch (e) {
      err0 = e;
    }
    expect(err0).toEqual(
      new Error('song does not exist. songId = dummySongId')
    );
    ...
    expect(downloads).toContainEqual(expect.objectContaining(song3));
